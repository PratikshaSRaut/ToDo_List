...
const countItems = (chores, filter) =>
  filter === "all"
    ? chores.length
    : chores.filter((item) => item.status === filter).length;
...
const renderList = (chores, filter = "all") => {
  ...
  chores.length > 0 &&
    chores.forEach((chore, iteration) => {
      if (filter == "all") {
...
const createListItem = (chore, id) => {
  return `
    ...
    chore.status === "complete" ? "checked" : ""
  } value="None" name=check-${id}  />
            <label class="status" for=check-${id}></label>
        </div>
    </section>
    <div  class="list-input ${chore.status === "complete" ? "strike" : ""} " >${
    chore.name
  }</div>
...
const addChore = (input) => {
  ...
  chore.length === 0
    ...
    : chores.find((item) => item.name == chore)
